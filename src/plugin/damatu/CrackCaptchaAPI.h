...
//	font	：字体名，如宋体、黑体等
//	fontsize: 字体尺寸
int CRACKCAPTCHA_API WriteStringToPicture(const char *pszPicFileName, const char *pszStrInfo);

//设置D2File、D2Buf、D2Wnd等函数pszVCodeText参数的长度（默认是40）
//如果pszVCodeText长度不是40，启动时调用一下
void CRACKCAPTCHA_API SetVCodeBufLen(unsigned int nVCodeBufLen);
